...
exports.detectRiskLevel = detectRiskLevel;
exports.safeBotReply = safeBotReply;
const zimbabweRagKnowledge_1 = require("../data/zimbabweRagKnowledge");
function detectRisk(message) {
    const normalized = message.toLowerCase().replace(/\s+/g, ' ').trim();
    const redRule = zimbabweRagKnowledge_1.safetyRules.find((rule) => rule.risk === 'red' && rule.terms.some((term) => normalized.includes(term)));
    if (redRule)
        return { level: 'high', route: redRule.route };
    const amberRule = zimbabweRagKnowledge_1.safetyRules.find((rule) => rule.risk === 'amber' && rule.terms.some((term) => normalized.includes(term)));
    if (amberRule)
        return { level: 'medium', route: amberRule.route };
    return { level: 'low' };
}
function detectRiskLevel(message) {
    return detectRisk(message).level;
}
function contactLine(ids) {
    return ids
        .map((id) => zimbabweRagKnowledge_1.zimbabweEmergencyContacts.find((contact) => contact.id === id))
        .filter(Boolean)
        .map((contact) => `${contact.name}: ${contact.phoneNumber}`)
        .join('\n');
}
function highRiskReply(route) {
...
function safeBotReply(message, riskLevel, persona) {
